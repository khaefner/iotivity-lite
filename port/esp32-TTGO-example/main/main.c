...
static lv_obj_t *light_on = NULL;
static lv_obj_t *light_off = NULL;
static void create_light_application();
static void create_qrcode_application(char *qrdata);
static void change_gui_state(bool state);
static char* qrdata = "cablelabs.com";
...
static void gpio_task(void* arg)
{
    ...
    uint32_t io_num;
    for(;;) {
	uint32_t button_ts =0;
        if(xQueueReceive(gpio_evt_queue, &io_num, portMAX_DELAY)) {
	    button_ts *= portTICK_PERIOD_MS;
	    //Debounce 
	    if(last < button_ts-300){
	    	printf("Got button press from GPIO[%d] button_ts[%d] last press[%d]\n",io_num,button_ts,last);
		last = button_ts;
		if (state == false){
			state=true;
		}else{
...
void button_init(void){
    ...
    printf("Button Config Initiated");
}
...
static void change_gui_state(bool state)
{
	if (state == 1){
		printf("Set Light ON\n");
		lv_obj_set_hidden(light_off, true);
		lv_obj_set_hidden(light_on, false);
	}else{
		printf("Set Light OFF\n");
		lv_obj_set_hidden(light_off, false);
		lv_obj_set_hidden(light_on, true);
	}
...
void app_main(void)
{
  ...
  xHandle = xTaskCreateStatic(
	...
	button_init();
	//xTaskCreatePinnedToCore(guiTask, "gui", 8096*2, NULL, 0, NULL, 1);
